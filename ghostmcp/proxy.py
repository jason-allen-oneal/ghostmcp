...
def apply_proxy_mode(command: list[str]) -> list[str]:
    ...
    if mode == "tor":
        # Environment proxy variables are not honored by many security tools.
        # Use an enforcement wrapper and fail closed if none is available.
        if shutil.which("torsocks"):
...
def validate_proxy_configuration(*, required: bool = False) -> None:
    ...
    if mode == "proxychains":
        ...
        raise ProxyConfigurationError(
            "Tor mode requires torsocks or proxychains"
        )
